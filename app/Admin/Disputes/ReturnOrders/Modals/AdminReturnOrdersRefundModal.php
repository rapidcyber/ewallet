...
class AdminReturnOrdersRefundModal extends Component
{
    use WithNumberGeneration, WithImage, WithBalance;
    ...
    public function refund()
    {
        ...
        $dispute_decision->type = 'refund';

        $admin_log = new AdminLog;
        $admin_log->user_id = auth()->id();
        $admin_log->title = 'Refunded return order ' . $this->return_order->id . ' for merchant ' . $this->merchant->id;
        ...
        try {
            ...
            $log->title = 'Admin resolved dispute - Refund only';
            $log->description = "Admin has resolved the dispute. Merchant has successfully processed a refund of " . $transaction->currency . number_format($refund_amount, 2) . " to buyer. Transaction reference number: " . $transaction->ref_no;
            $log->save();
            ...
            DB::rollBack();
            Log::error('AdminReturnOrdersRefundModal.refund - ' . $th->getMessage());
    ...
    {
        return view('admin.disputes.return-orders.modals.admin-return-orders-refund-modal');
    }
}
