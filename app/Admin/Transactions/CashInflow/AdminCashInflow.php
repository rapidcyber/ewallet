...
namespace App\Admin\Transactions\CashInflow;
...
class AdminCashInflow extends Component
{
    ...
    public $orderPaymentCount = 0;

    public $billPaymentCount = 0;
    ...
    {
        return TransactionType::whereIn('code', ['TR', 'OR', 'BP'])->toBase()->get();
    }
    ...
    public function updatedDateFilter()
    {
        ...
        $type_order = $this->transaction_types->where('code', 'OR')->first();
        $type_bill = $this->transaction_types->where('code', 'BP')->first();
        ...
        $this->moneyTransferCount = $user->incoming_transactions()
            ...
            ->whereBetween('created_at', [$fromDate, $toDate])
            ->count();
        $this->orderPaymentCount = $user->incoming_transactions()
            ->where('transaction_type_id', $type_order->id)
            ->whereBetween('created_at', [$fromDate, $toDate])
            ->count();
        $this->billPaymentCount = $user->incoming_transactions()
            ->where('transaction_type_id', $type_bill->id)
            ->whereBetween('created_at', [$fromDate, $toDate])
            ->count();
    }
