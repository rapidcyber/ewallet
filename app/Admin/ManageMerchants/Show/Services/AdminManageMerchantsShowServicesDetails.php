...
class AdminManageMerchantsShowServicesDetails extends Component
{
    ...
    public function change_status()
    {
        ...
        try {
            ...
            $notif->recipient_id = $this->service->merchant->id;
            $notif->recipient_type = Merchant::class;
            $notif->ref_id = $this->service->id;
            ...
            switch ($this->actionType) {
                case 'approve':
                    ...
                    $notif->message = "Your service, {$this->service->name}, has been approved.";
                    break;
                case 'deny':
                    $this->service->approval_status = 'rejected';
                    $log->title = 'Rejected service ' . $this->service->id;
                    $notif->message = "Your service, {$this->service->name}, has been rejected.";
                    break;
                case 'reactivate':
                    $this->service->approval_status = 'approved';
                    $log->title = 'Reactivated service ' . $this->service->id;
                    $notif->message = "Your service, {$this->service->name}, has been reactivated.";
                    break;
                case 'suspend':
                    $this->service->approval_status = 'suspended';
                    $log->title = 'Suspended service ' . $this->service->id;
                    $notif->message = "Your service, {$this->service->name}, has been suspended.";
                    break;
            }
