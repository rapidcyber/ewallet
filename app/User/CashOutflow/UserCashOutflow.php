...
class UserCashOutflow extends Component
{
    ...
    #[Locked]
    public $allowedOrderByFieldName = [
        'total_amount',
        'created_at',
    ];
    #[Locked]
    public $allowedFilterBox = [
        'all',
        'transfer',
        'order_payment',
        'invoice_payment',
    ...
    public function updatedDateFilter()
    {
        ...
        if ($dateFilter === 'past_year') {
            ...
            return session()->flash('error', 'Invalid date filter');
        }
    ...
    public function render()
    {
        ...
        $cashOutflows = $this->user->outgoing_transactions()
            ...
            ->with(['type', 'status', 'recipient' => function (MorphTo $q) {
                $q->morphWith([
                    User::class => ['profile'],
                ]);
            }]);

        if ($this->activeBox && in_array($this->activeBox, $this->allowedFilterBox) && $this->activeBox !== 'all') {
        ...
        if ($searchTerm !== '') {
            $cashOutflows = $cashOutflows->where(function ($query) use ($searchTerm) {
                ...
                $query->orWhereHasMorph('recipient', [Merchant::class], function ($merchant) use ($searchTerm) {
                    ...
                    $merchant->orWhere('account_number', 'LIKE', '%' . $searchTerm . '%');
                });
        ...
        return view('user.cash-outflow.user-cash-outflow')->with([
