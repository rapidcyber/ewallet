<x-main.content class="!px-16 !py-10" x-data="{
    ...
    <x-main.title class="mb-8">Manage Products</x-main.title>

    <div class="grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 mb-8">
        ...
        <button wire:click='$set("selectedBox", "all")' class="{{ $selectedBox == 'all' ? 'bg-purple-gradient-to-right shadow-lg z-10 text-white' : 'bg-white hover:shadow-lg transition-all text-primary-600' }} flex flex-row items-center justify-between cursor-pointer px-4 py-6 rounded-1 break-words">
            <div class="flex items-center gap-2">
                ...
                <x-icon.cube-box width="24" height="24" fill="{{ $selectedBox == 'all' ? '#ffff' : '#7f56d9' }}" />
                <p class="w-28 {{ $selectedBox == 'all' ? 'text-white' : 'text-rp-neutral-600' }}">All products</p>
            </div>
        ...
        <x-table.standard class="break-words">
            <x-slot:table_header>
                <x-table.standard.th class="w-10">
                    <x-input type="checkbox" wire:model.live="selectAll" wire:change="handleSelectAllCheckbox($event.target.checked, {{$products->pluck('id')}})" />
                </x-table.standard.th>
                @if (count($checkedProducts) > 0)
                    <x-table.standard.th class="w-52 min-w-52 max-w-52">
                        ({{ count($checkedProducts) }} selected)
                    </x-table.standard.th>
                    <x-table.standard.th class="w-52 min-w-52 max-w-52">
                        {{-- Blank --}}
                    </x-table.standard.th>
            ...
            <x-slot:table_data>
                @foreach ($products as $product)
                    <x-table.standard.row wire:key="{{ $product->id }}">
                        <x-table.standard.td>
                            <x-input type="checkbox" wire:model.live="checkedProducts" value="{{ $product->id }}" wire:change="handleSingleSelectCheckbox({{ $products->getCollection() }})" />
                        </x-table.standard.td>
                        <x-table.standard.td class="w-52 min-w-52 max-w-52">
                            <a  href="{{ route('admin.manage-products.show', ['product' => $product]) }}" class="hover:underline">
                                {{ $product->name }}
                            </a>
                        </x-table.standard.td>
                        <x-table.standard.td class="w-52 min-w-52 max-w-52">
                        ...
                        <x-table.standard.td class="w-52 min-w-52 max-w-52">
                            {{ \Carbon\Carbon::parse($product->created_at)->timezone('Asia/Manila')->format('Y-m-d') }}
                        </x-table.standard.td>
                        ...
                        <x-table.standard.td class="w-52 min-w-52 max-w-52 2xl:min-w-80 2xl:max-w-80">
                            <div class="flex items-center w-full gap-1">
                                <div class="flex xl:flex-col w-5/6 2xl:flex-row gap-1">
                                    @if (count($this->checkedProducts) === 0)
                                        @switch($product->approval_status)
                                            @case('review')
                                                <x-button.filled-button @click="$wire.set('product_id',{{ $product->id }});confirmationModal.actionType='approve';confirmationModal.visible=true" color="primary" class="xl:w-full 2xl:w-1/2">approve</x-button.filled-button>
                                                <x-button.outline-button @click="$wire.set('product_id',{{ $product->id }});confirmationModal.actionType='deny';confirmationModal.visible=true" color="primary" class="xl:w-full 2xl:w-1/2">deny</x-button.outline-button>
                                                @break
                                            @case('approved')
                                                <x-button.filled-button @click="$wire.set('product_id',{{ $product->id }});confirmationModal.actionType='deactivate';confirmationModal.visible=true" color="primary" class="xl:w-full 2xl:w-1/2">deactivate</x-button.filled-button>
                                                @break
                                            @case('rejected')
                                                <x-button.filled-button @click="$wire.set('product_id',{{ $product->id }});confirmationModal.actionType='reactivate';confirmationModal.visible=true" color="primary" class="xl:w-full 2xl:w-1/2">reactivate</x-button.filled-button>
                                                @break
                                            @case('suspended')
                                                <x-button.filled-button @click="$wire.set('product_id',{{ $product->id }});confirmationModal.actionType='reactivate';confirmationModal.visible=true" color="primary" class="xl:w-full 2xl:w-1/2">reactivate</x-button.filled-button>
                                                @break
                                ...
                                <div class="w-max">
                                    <a  href="{{ route('admin.manage-products.show', ['product' => $product]) }}"><x-icon.chevron-right class="w-full" /></a>
                                </div>
                            </div>
                        </x-table.standard.td>
    ...
    <div class="w-full flex items-center justify-center gap-8">
        @if ($products->hasPages())
            <div class="flex flex-row items-center h-10 gap-0 mt-4 w-max border rounded-md overflow-hidden">
                <button wire:click="previousPage" {{ $products->onFirstPage() ? 'disabled' : '' }} @click="$wire.set('checkedProducts', [])"
                ...
                @foreach ($elements as $element)
                    ...
                    @else
                        <button wire:click="gotoPage({{ $element }})" @click="$wire.set('checkedProducts', [])"
                ...
                <button wire:click="nextPage" {{ !$products->hasMorePages() ? 'disabled' : '' }} @click="$wire.set('checkedProducts', [])"
    ...
    <x-modal x-model="groupConfirmationModal.visible">
        <x-modal.confirmation-modal title="Confirmation">
            <x-slot:message>
                Are you sure you want to <span x-text="groupConfirmationModal.actionType"></span> these products?
            </x-slot:message>
            <x-slot:action_buttons>
                <x-button.outline-button
