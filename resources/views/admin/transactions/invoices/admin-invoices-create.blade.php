<div class="flex h-full" x-data="{ isAddItemVisible: $wire.entangle('isAddItemVisible'), isEditItemModalVisible: $wire.entangle('isEditItemModalVisible') }">
        ...
        <div>
                    ...
                    <div class="mt-5 overflow-y-auto max-h-64 py-2">
                        @foreach ($results as $result)
                            <div class="flex flex-row items-center justify-between border border-black bg-white rounded-lg px-5 py-4 cursor-pointer mb-2"
                                ...
                                <p>{{ $result['name'] }}</p>
                                <p>{{ $this->format_phone_number($result['phone_number'], $result['phone_iso']) }}</p>
                            </div>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="flex gap-3 mb-3">
                <x-input.input-group class="w-1/2">
                    ...
                    <x-dropdown.select wire:model.blur='currency' class="h-11">
                ...
                <x-input.input-group class="w-1/2">
                    ...
                    <x-input wire:model.blur='due_date' type="date" class="h-full" />
                    @error('due_date')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </x-input.input-group>
            ...
            <div class="mb-3">
                ...
                <div class="flex flex-col gap-2 mt-3">
                    ...
                    <x-button.outline-button wire:click="handleAddItemModal(true)" color="primary">add</x-button.outline-button>
                </div>
            </div>
            ...
            <x-slot:profile>
                <x-layout.summary.profile
                    ...
                    <x-slot:info_block_bottom>
                        {{ $this->format_phone_number($recipient['phone_number'], $recipient['phone_iso']) }}
                    </x-slot:info_block_bottom>
                </x-layout.summary.profile>
            </x-slot:profile>
            <x-slot:body>
                <x-layout.summary.section title="Invoice Details" color="primary">
                    <x-slot:data>
                ...
                <x-layout.summary.section title="Items & Services" color="primary">
                    <x-slot:data>
                        ...
                        <x-layout.summary.label-data>
                            ...
                            <x-slot:data class="text-primary-600">
                                {{ '₱' . number_format($this->total_items, 2) }}
                            </x-slot:data>
                        </x-layout.summary.label-data>
                    </x-slot:data>
                </x-layout.summary.section>
                @if ($allow_vat || $allow_discount || $allow_shipping)
                    <x-layout.summary.section title="Inclusions" color="primary">
                        <x-slot:data>
                            @if ($allow_vat)
                                <x-layout.summary.label-data>
                                    <x-slot:label>
                ...
                @endif
                <x-layout.summary.section title="Summary" color="primary">
                    <x-slot:data>
                        <x-layout.summary.label-data>
                            <x-slot:label>
                                Items & Services
            ...
            <x-slot:action>
                ...
                <div class="flex flex-col gap-3">
                    <x-button.filled-button wire:click="submit" wire:target='submit' wire:loading.attr='disabled' wire:loading.class='opacity-50' :disabled="!$agree"
                        size="md" color="primary">send</x-button.filled-button>
            ...
            <x-slot:action>
                ...
                <div class="flex flex-col gap-3">
                    <x-button.filled-button :disabled="!$agree"
                        size="md" color="primary">send</x-button.filled-button>
                    <x-button.outline-button @click="history.back()" size="md"
                        color="primary">cancel</x-button.outline-button>
                </div>
            </x-slot:action>
        @endif
        ...
        <x-modal.form-modal title="Add Item">
            ...
            <x-slot:action_buttons>
                <x-button.outline-button wire:target='handleCommitAdd' wire:loading.attr="disabled"
                    ...
                    class="w-1/2" color="primary">cancel</x-button.outline-button>
                <x-button.filled-button wire:click="handleCommitAdd()" wire:target='handleCommitAdd'
                    wire:loading.attr="disabled" wire:loading.class="opacity-50"
                    class="w-1/2" color="primary">add</x-button.filled-button>
            </x-slot:action_buttons>
        </x-modal.form-modal>
    </x-modal>
        ...
        <x-modal.form-modal title="Edit Item">
            ...
            <x-slot:action_buttons>
                <x-button.outline-button @click="isEditItemModalVisible=false"
                    class="w-1/2" color="primary">cancel</x-button.outline-button>
                <x-button.filled-button wire:click="handleCommitEdit()" class="w-1/2" color="primary">add</x-button.filled-button>
            </x-slot:action_buttons>
        </x-modal.form-modal>
    </x-modal>
    ...
    <div x-data="{ show: @entangle('apiErrorMsg') }" x-cloak x-show="show"
        ...
        <div class="p-5 bg-white border-2 border-red-600 rounded-md w-96">
            <div class="flex flex-row justify-between items-center p-4">
                <p class="text-red-600 text-lg">Transaction Failed</p>
                <div>
                    <x-icon.error />
                </div>
            </div>
    ...
    <x-loader.black-screen wire:loading wire:target="handleRecipientSelection" />
</div>
