<div class="flex h-full">
    <x-main.content class="overflow-y-auto grow !px-[18.5px]">
        ...
        <x-card.display-balance :balance="$this->available_balance" color="primary" class="mb-8" />
    ...
    <x-layout.summary>
        ...
        <x-slot:body>
            @if ($transaction_type === 'money-transfer')
                @if ($transfer_to === 'another-account')
                    <x-layout.summary.section title="Payment Details" color="primary">
                        <x-slot:data>
                    ...
                    @if (!empty($message))
                        <x-layout.summary.section title="Message" color="primary">
                            <x-slot:data>
                                <p>{{ $message }}</p>
                            </x-slot:data>
                        </x-layout.summary.section>
                ...
                @elseif ($transfer_to === 'unionbank-account')
                    ...
                    <x-layout.summary.section title="Notification Details" color="primary">
                        <x-slot:data>
                            <x-layout.summary.label-data>
                                <x-slot:label>Phone Number</x-slot:label>
                    ...
                    <x-slot:data>
                        ...
                        @if (!empty($amount))
                            ...
                            <x-layout.summary.label-data>
                                ...
                                <x-slot:data class="text-primary-500">
        ...
        <x-slot:action>
            ...
            <div class="flex flex-col gap-3">
                <x-button.filled-button wire:click="submit" :disabled="$this->button_submit_clickable == false" form="cash-outflow-form" color="primary"
                    size="md">send</x-button.filled-button>
                <x-button.outline-button href="{{ route('admin.transactions.cash-outflow.index') }}" size="md" color="primary">cancel</x-button.outline-button>
            </div>
        </x-slot:action>
    </x-layout.summary>

    {{-- Loader --}}
    {{-- @TODO: Put wire:taget to specifically target function to trigger the loader --}}
    <x-loader.clock wire:loading wire:target="transaction_type,transfer_to,submit,get_biller_input_fields" />
