<x-main.content x-data="{
    showPreviousWorkModal: $wire.entangle('showPreviousWorkModal'),
    ...operatingDays(),
}">
    ...
    <div class="bg-white flex flex-col rounded-lg gap-4 px-4 py-5 w-full mt-7">
        ...
        <div class="operatingDaysSlider !overflow-hidden{{-- grid grid-cols-3 w-full text-sm --}}" wire:ignore>
            <div class="swiper-wrapper">
                <template x-for="(day, day_index) in operatingDays" :key="day.id">
                    <div class="swiper-slide p-2">
                        ...
                        {{-- Time slots --}}
                        <div class="flex flex-col max-w-[318px] gap-3" :class="day.checked === false && 'opacity-50 pointer-events-none'">
                            <template x-for="(slot, timeslot_index) in day.timeslots">
                                <div>
                                    <h5 class="font-medium text-rp-neutral-700 mb-7">Time slot <span
                                            x-text="timeslot_index+1"></span></h5>

                                    <div class="flex flex-row gap-1 justify-between items-center">
                                        <div class="relative">
                                            <p class="absolute bottom-[100%]"><span class="text-[#E31C79]">*</span>Start
                                                time</p>

                                            <x-dropdown.select x-model="slot['start_time'].selected" x-ref="`start_time_${day_index}_${timeslot_index}`">
                                                <x-dropdown.select.option value="" selected disabled>---</x-dropdown.select.option>
                                                <template x-for="(time, start_time_index) in slot['start_time'].option">
                                                    <x-dropdown.select.option ::value="time" ::selected="time === slot['start_time'].selected"
                                        ...
                                        <div>-</div>
                                        <div class="relative">
                                            <p class="absolute bottom-[100%]"><span class="text-[#E31C79]">*</span>End
                                                time</p>

                                            <x-dropdown.select x-model="slot['end_time'].selected">
                                                <x-dropdown.select.option value="" selected disabled>---</x-dropdown.select.option>
                                                <template x-for="time in slot['end_time'].option">
                                                    <x-dropdown.select.option ::value="time" ::selected="time === slot['end_time'].selected"
                                                        x-text="time"></x-dropdown.select.option>
                                                </template>
                                            </x-dropdown.select>
                                        </div>
                                        {{-- Remove --}}
                                        <div @click="removeTimeslot({ dayNum: day.id, slotIndex: timeslot_index })"
                                            ...
                                            <svg  width="20" height="22"
                                                ...
                                                <path
                                                    ...
                                                    stroke="#647887" stroke-width="1.5" stroke-linecap="round"
                                                    stroke-linejoin="round" />
                                                <path d="M8.33008 15.5H11.6601" stroke="#647887" stroke-width="1.5"
    ...
    <div class="bg-white flex flex-col rounded-lg gap-4 px-4 py-5 w-full mt-7">
        ...
        <div x-ref="questions" class="space-y-6" x-data="{ handleQuestionSort: (item, position) => { $wire.sortQuestion(item, position) } }" x-sort="handleQuestionSort" x-sort.ghost>
            @foreach ($service_form as $key => $question)
                <div x-sort:item="{{ $key }}" class="bg-rp-neutral-50 px-4 py-3 rounded-md"
                    ...
                    @if (in_array($question['type'], ['multiple', 'checkbox', 'dropdown']))
                        <div class="flex flex-row items-start justify-between text-sm">
                                            ...
                                            <div class="w-full space-y-2">
                                                @foreach ($question['choices'] as $choiceKey => $choice)
                                                    <div class="flex flex-row items-center gap-2"
                                                        ...
                                                        @if ($loop->count !== 1)
                                                            {{-- Delete --}}
                                                            <div wire:click="removeQuestionChoice({{ $key }}, {{ $choiceKey }})"
                                                                class="cursor-pointer">
                                                                <x-icon.close />
                                                            </div>
                                                        @endif
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @if ($loop->count !== 1)
                                    {{-- Delete --}}
                                    <div wire:click="removeQuestion({{ $key }})" class="cursor-pointer">
                                        <x-icon.trash />
                                    </div>
                                @endif
                            </div>
                    ...
                    @elseif ($question['type'] === 'paragraph')
                        ...
                        <div class="flex flex-row items-start justify-between text-sm">
                            <div class="flex flex-row gap-4">
                                ...
                                <div class="w-80">
                                    <p>Question title</p>
                                    <x-input wire:model="service_form.{{ $key }}.question" type="text" />
                                </div>
                                {{-- Question type --}}
                                <div class="w-56">
                                    <p>Question type</p>
        ...
        @error('service_form')
            <p class="text-xs text-red-500">{{ $message }}</p>
        @enderror
        @foreach ($errors->get('service_form.*') as $message)
            <p class="text-xs text-red-500">{{ $message[0] }}</p>
        @endforeach
    </div>
    ...
    <div class="flex flex-col gap-4 mb-9 p-6 rounded-lg bg-white mt-7">
        ...
        <form wire:submit.prevent='getMapLatlng' id="searchAddress"
            class="relative flex border rounded-lg overflow-hidden bg-white items-center w-full">
            <x-input.search wire:model.live='location' class="w-full" id="inp_search" icon_position="right" />
        </form>
        <div>
            <div wire:ignore id="map" class="w-full h-[570px] border rounded-lg"></div>
        </div>
        @error('location')
            <p class="text-xs text-red-500">{{ $message }}</p>
        @enderror
    ...
    <div class="bg-white rounded-lg gap-4 px-4 py-5 w-full mt-7">
        ...
        <div class="flex flex-col w-full gap-10">
            ...
            @foreach ($previous_works as $key => $previous_work)
                <div class="flex justify-between items-center w-full" wire:key='previous-work-{{ $key }}'>
    ...
    <div class="w-full flex justify-center mt-5 gap-2">
        {{-- Submit --}}
        <x-button.filled-button wire:target="save" wire:loading.attr='disabled' wire:loading.class='cursor-progress'
            @click="submitForm" class="w-56">submit</x-button.filled-button>
        {{-- Cancel --}}
        <x-button.outline-button wire:target="save" wire:loading.attr='disabled' wire:loading.class='cursor-progress'
            class="w-56">cancel</x-button.outline-button>
    </div>

    {{--  --------- MODAL STARTS HERE ---------- --}}
    {{-- Add previous work --}}
    <x-modal x-model="showPreviousWorkModal" class="!z-[1000]">
        <x-modal.form-modal title="Add previous work"
            @click.outside="showPreviousWorkModal=false;$wire.cancelPreviousWork();">
            {{-- FORM --}}
            <form action="" class="flex flex-col gap-2">
                {{-- Services images --}}
                <livewire:components.input.interactive-upload-images :uploaded_images="$previous_work_images" :max="5"
                    function="updatePreviousWorkImages" />
            ...
            <x-slot:action_buttons>
                ...
                <x-button.filled-button @click="$wire.addPreviousWork();"
    ...
    <x-loader.black-screen wire:loading.flex wire:target="save,service_form.*.type" class="z-[1000]" />
</x-main.content>
