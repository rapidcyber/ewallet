<div class="absolute flex flex-col gap-8 bg-white p-10 rounded-2xl w-[1180px] max-w-[90%] max-h-[95%] overflow-y-auto">
    ...
    {{-- HEADING --}}
    <div>
        <h3 class="text-2xl font-bold mb-2">Respond</h3>
    </div>
    ...
    {{-- FORM --}}
    <form wire:submit.prevent="submit_response" class="flex flex-col gap-2">

        {{-- comment --}}
        <x-input.input-group>
            <x-slot:label for="comment"><span class="text-rp-red-600"></span>*Comment</x-slot:label>
            <x-input.textarea x-ref="inpcomment" name="comment" wire:model.live="comment" id="comment" cols="30"
                rows="8" maxlength="500"></x-input.textarea>
        ...
        <div class="grid grid-cols-2 gap-3">
            <x-button.outline-button type="button" @click="$dispatch('closeModal')" wire:target="refund"
                wire:loading.attr="disabled" wire:loading.class="opacity-50">Cancel</x-button.outline-button>
            <x-button.filled-button type="submit" wire:target="refund" wire:loading.attr='disabled'
