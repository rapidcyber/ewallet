    ...
    <x-layout.details.more-details>
        <x-layout.details.more-details.section title="General Details">
            <div class="space-y-2">
                ...
                <x-layout.details.more-details.data-field field="Transaction Date" value="{{ \Carbon\Carbon::parse($dispute->transaction->created_at)->format('m/d/Y') }}" />                
                <x-layout.details.more-details.data-field field="Transaction Amount" value="₱{{ number_format($dispute->transaction->amount, 2) }}" />
                <x-layout.details.more-details.data-field field="Transaction Reference Number" value="{{ $dispute->transaction->ref_no }}" />
                <x-layout.details.more-details.data-field field="Date Created" value="{{ \Carbon\Carbon::parse($dispute->created_at)->format('m/d/Y') }}" />
            </div>
