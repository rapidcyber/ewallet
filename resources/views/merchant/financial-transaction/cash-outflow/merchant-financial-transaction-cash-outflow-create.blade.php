    ...
    <x-main.content class="overflow-y-auto grow !px-[18.5px]">
                    ...
                    <div class="flex flex-row justify-between gap-2">
                        <x-input.input-group class="flex-1">
                            ...
                            <div class="flex flex-row gap-2">
                                ...
                                <x-input type="text" id="phone_number" wire:model.lazy="phone_number" maxlength="15"
        ...
        @elseif ($transaction_type === 'bill-payment')
            ...
            <div class="flex flex-row justify-between gap-2">
                <x-input.input-group class="flex-1">
                    <x-slot:label for="amount">Amount:</x-slot:label>
                    <x-input wire:model.blur="amount" type="text" maxlength="8" id="amount"
                ...
                <x-input.input-group class="flex-1">
                    <x-slot:label for="email-receipt">Email receipt to (optional):</x-slot:label>
                    <x-input type="text" maxlength="255" wire:model.blur="email" id="email-receipt">
                        <x-slot:icon>
                            <x-icon.mail />
                        </x-slot:icon>
                    </x-input>
    ...
    <x-layout.summary>
                    ...
                    <x-slot:info_block>
                        @if (!empty($biller_type))
                            <div class="gap-3 text-center">
                                ...
                                <p class="text-sm">{{ $this->get_biller_category }}</p>
                        ...
                        @if (!empty($amount))
                            ...
                            <x-layout.summary.label-data>
                                ...
                                <x-slot:data class="text-rp-red-500">
                                    {{ \Number::currency($amount + $this->get_biller_service_charge, 'PHP') }}
                                </x-slot:data>
                            </x-layout.summary.label-data>
                        @endif
                    </x-slot:data>
        ...
        <x-slot:action>
            ...
            <div class="flex flex-col gap-3">
                <x-button.filled-button @click="onSubmit" :disabled="$this->button_submit_clickable == false" form="cash-outflow-form" size="md">
                    @if ($transaction_type === 'bill-payment' and $can_pay_bill == false)
                        create bill
                    @else
                        send
                    @endif
                ...
                @if ($transaction_type === 'bill-payment')
                    <x-button.outline-button href="{{ route('merchant.financial-transactions.bills.index', ['merchant' => $merchant]) }}" size="md" color="red">cancel</x-button.outline-button>
                @else    
                    <x-button.outline-button href="{{ route('merchant.financial-transactions.cash-outflow.index', ['merchant' => $merchant]) }}" size="md" color="red">cancel</x-button.outline-button>
                @endif
            </div>
        </x-slot:action>
    </x-layout.summary>
