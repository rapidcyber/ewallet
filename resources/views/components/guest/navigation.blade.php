...
<div wire:ignore class="max-w-6xl flex flex-row justify-between items-center bg-transparent w-full p-9"
    ...
    <ul class="{{ $whiteText ? 'text-white' : '' }} flex-row gap-6  font-thin hidden md:flex">
        ...
        @else
            @guest
                <li>
                    <a href="{{ route('sign-in') }}" class="{{ request()->routeIs('sign-in') ? 'underline font-bold' : '' }}">Sign-in</a>
                </li>
            @endguest
            @auth
                <li>
                    <div role="button" tabindex="0" @keyup.enter="isLogoutModalVisible=true;"
                        @click="setLogoutVisible">Logout</div>
                </li>
            @endauth
        @endif
    </ul>

    <div role="button" tabindex="0" @keyup.enter="isMobileMenuVisible=true" class="cursor-pointer block md:hidden"
    ...
    <template x-teleport="body">
        <div x-cloak x-show="isMobileMenuVisible" class="fixed inset-0 bg-black bg-opacity-50 md:hidden">
            <div @click.away="isMobileMenuVisible=false"
        ...
        <x-modal x-model="isLogoutModalVisible">
            <x-modal.confirmation-modal title="Logout" message="Are you sure you want to log out?">
                <x-slot:action_buttons>
                    <x-button.outline-button class="w-1/2" @click="setLogoutVisible">Cancel</x-button.outline-button>
